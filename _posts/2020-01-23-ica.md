...
PCA finds the directions that maximize  $$\kappa_2$$ whereas ICA has to do with maximization of higher order cumulants. In that sense there are many ICAs associated with whatever cumulant is being maximized. We begin with the variant of ICA that maximizes  $$|\kappa_4|$$  and show that ICA, via kurtosis maximization, does indeed recover the independent components.
...
What happens when we maximize $$|\kappa_4|$$ or equivalently use abs.kurtosis based ICA?
...
$$\pm e_i$$ for $$i = \arg \max_j | \kappa_4 ( x_j )|$$
...
subject to $$|| w \parallel_{2} = 1$$ and $$w \perp q_1, \ldots, q_{i-1}$$
