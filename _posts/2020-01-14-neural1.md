...
The labeling is arbitrary, but once we pick it we must stick with it.

Now we need training data for the two classes, so our neural network can learn how to distinguish them.

**Setup**

Suppose we have a training dataset comprised of  𝑛 -dimensional vectors  $$[𝑥^{"0"}_1 ,𝑥^{"0"}_2 … 𝑥^{"0"}_{𝑁0} ]$$  asssociated with the Class 0 category. (The superscript in $$ 𝑥^{"0"}_1 $$  indicates that this is a vector of inputs belonging to Class 0.)

Similarly, we have a training dataset comprised of  𝑛 -dimensional vectors  $$ [𝑥^{"1"}_1 ,𝑥^{"1"}_2 … 𝑥^{"1"}_{𝑁1} ]  associated with Class 1. (As before, the superscript reminds us that $$ 𝑥^{"1"}_1  is a vector of inputs belonging to Class 1.)

These two sets of vectors with known classifications contain the "meanings" of Class 0 and Class 1, and we can train a neural network to use these training vectors to distinguish between the two classes.

> **Objective**
> Our objective is to train a neural network to distinguish between Class 0 and Class 1.

### 2.2 The mean-squared error loss function
...
### 2.3 Gradient of the loss function
...
### 2.4 Learning network parameters using gradient descent
...
### 2.5 Faster learning with stochastic gradient descent
...
### 2.6 Nesterov's accelerated gradient descent method
...
$$\gamma_k = \dfrac{1-\lambda_k}{\lambda_{k+1}},$$

$$\lambda_0 = 0 \textrm{ and } \lambda_{k+1} = \dfrac{1+\sqrt{1+4\,\lambda_{k}^2}}{2}.$$
